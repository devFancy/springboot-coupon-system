...
* 로그인
    ...
    * 로그인 시 아이디와 비밀번호가 필요하다.

### Coupon (쿠폰 정의)

속성(상태)

* 쿠폰에는 이름, 타입, 상태, 발급 수량, 유효 시작일, 유효 종료일이 있다.

    * 타입: CHICKEN, HAMBURGER, PIZZA
    * 상태: ACTIVE, EXPIRED, DISABLED

행위(기능)

* 관리자는 쿠폰을 생성할 수 있다.
* 쿠폰은 발급 시 수량을 차감할 수 있다.
* 유효기간에 따라 상태를 변경할 수 있다.

정책(제약사항)

* 발급 가능 수량은 1 이상이어야 한다.
* 유효 시작일과 종료일은 반드시 설정되어야 한다.
* 유효 종료일이 지나면 상태는 EXPIRED로 바뀔 수 있다.
...
* 동일 사용자에게 동일 쿠폰은 1회만 발급 가능하다 → userId + couponId 유니크
* 사용된 쿠폰은 재사용이 불가능하다.
